<template>
  <v-container>
    <v-card class="pa-5 mx-auto" max-width="1200">
      ...
      <v-card-title class="text-h5">Employee List <v-spacer></v-spacer>  <v-btn icon="mdi-plus" @click="openAddDialog"></v-btn> </v-card-title>
      ...
      <v-card-text>
        ...
        >
          ...
          <template v-slot:[`item.delete`] = "{ item }"> <v-icon small @click="deleteEmployee(item)">mdi-delete</v-icon>
          </template>
    ...
    <v-card class="pa-5 mx-auto mt-5" max-width="500">
...
export default {
  ...
  computed: {
    filteredEmployees() {  // Computed property for filtered employees
      const searchTerm = this.search.toLowerCase();
      return this.employees.filter(employee => {
        // Customize the search fields as needed
        const searchableFields = [
          employee.name,
          employee.email,
          employee.phone, // Include phone number in search
  ...
  methods: {
    ...
    closeEditDialog() {
      ...
      this.editedEmployee = { // Reset the editedEmployee object
        id: null,
        name: "",
        email: "",
        phone: "",
      ...
      if (this.$refs.editForm) { // Check if the ref exists
    ...
    async saveEmployee() {
      if (this.editFormValid) {
        try {
          ...
          const response = await EmployeeApi.updateEmployees(this.editedEmployee.id, payload);
